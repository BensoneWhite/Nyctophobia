...
public class BloodyFlowerPlacer : UpdatableAndDeletable
{
    public BloodyFlowerPlacer(Room room, PlacedObject placedObject)
    {
        if (room.abstractRoom.firstTimeRealized)
        {
            ...
            {
                AbstractConsumable obj = new(room.world, NTEnums.AbstractObjectTypes.BloodyKarmaFlower, null, room.GetWorldCoordinate(placedObject.pos), room.game.GetNewID(), room.abstractRoom.index, objIndex, new PlacedObject.ConsumableObjectData(placedObject))
                {
                    isConsumed = false,
